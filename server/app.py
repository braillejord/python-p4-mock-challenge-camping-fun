...
class Campers(Resource):
    def get(self):
        campers = [
            camper.to_dict(rules=("-activities", "-signups")) for camper in Camper.query.all()
        ]

        if campers is None:
            return make_response({"error": "No campers found."}, 404)
    ...
    def post(self):
        ...
        try:
            new_camper = Camper(name=fields["name"], age=fields["age"])

            db.session.add(new_camper)
            db.session.commit()

            return make_response(new_camper.to_dict(rules=("-signups", "-activities")), 201)
...
class CampersById(Resource):
    ...
    def patch(self, id):
        ...
        try:
            ...
            # return make_response(camper.to_dict(rules=("-signups", "-activities")), 202)
...
class Signups(Resource):
    def post(self):
        ...
        try:
            new_signup = Signup(
                camper_id=data["camper_id"], activity_id=data["activity_id"], time=data["time"]
            )
